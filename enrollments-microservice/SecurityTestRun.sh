...
docker run --network host -v $(pwd)/zap-wrk:/zap/wrk -v $(pwd)/zap-reports:/zap/reports -t zaproxy/zap-stable zap-baseline.py -t http://192.168.3.82:8004 -r SecurityTestReport.html > SecurityTestReport.txt
